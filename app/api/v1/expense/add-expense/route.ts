...
import Expense from '@/models/expenseSchema'; // Adjusted import

export async function POST(req: NextRequest) {
    await connectDb(); // Ensure database connection is awaited
    try {
        const body = await req.json();
        const result = await Expense.create(body); // Use create method directly on the model
        ...
        return NextResponse.json({ message: 'Expense record created successfully', income: result }, { status: 201 });
    } catch (error) {
        console.error(error);
